...
from fdu_mcp_server.modules.data_types import (
    ImportPlayersCommand, 
    AddPlayerCommand, 
    ListPlayersCommand
)
from fdu_mcp_server.modules.functionality.import_players import import_players
...
def test_import_players(temp_db_path):
    ...
    with tempfile.NamedTemporaryFile(suffix='.csv', mode='w', delete=False, newline='') as csv_file:
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(suffix='.csv', mode='w', delete=False, newline='') as update_file:
            update_path = Path(update_file.name)
            writer = csv.writer(update_file)
            writer.writerow(['Nombre', 'Telefono', 'Email'])
            writer.writerow(['Player 1', '+9999999999', 'updated@example.com'])  # Updated
            writer.writerow(['Player 4', '+4444444444', 'player4@example.com'])  # New
        
        try:
            # Create the update command
            update_command = ImportPlayersCommand(
                csv_path=update_path,
                db_path=temp_db_path
            )
            ...
            updated_player1 = next(player for player in updated_db_players if player.name == "Player 1")
            ...
            player4 = next(player for player in updated_db_players if player.name == "Player 4")
        ...
        with tempfile.NamedTemporaryFile(suffix='.csv', mode='w', delete=False, newline='') as error_file:
        ...
        try:
            ...
            player5 = next(player for player in final_db_players if player.name == "Player 5")
